namespace VehiclesExtension.Core
{
    ...
    public class Engine
    {
        public void Run()
        {
            ...
            for (int i = 0; i < count; i++)
            {
                try
                {
                    ...
                    if (command == "Drive")
                    {
                        PrintDriveCommand(car, truck, bus, commandItems, type);
                    }
                    else if (command == "Refuel")
                    {
                        PrintRefuelCommand(car, truck, bus, commandItems, type);
                    }
        ...
        private static void PrintDriveCommand(Car car, Truck truck, Bus bus, string[] commandItems, string type)
        {
            double distance = double.Parse(commandItems[2]);
